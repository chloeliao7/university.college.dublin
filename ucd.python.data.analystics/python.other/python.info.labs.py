...
english = ["hello", "cat", "dog", "yes", "tomorrow", "yesterday", "difficult", "easy", "bad", "no", "tuesday", "january"]
french = ["bonjour", "chat", "chien", "oui", "demain", "hier", "difficile", "facile", "mauvais", "non", "mardi", "janvier"]
if(len(english) % 2 == 0 and len(french) % 2 == 0): print("They are both even");
else:
...
for counter in range(len(english)): english_and_french[english[counter]] = french[counter];
...
def translate(dictionary, word_to_translate, default_word): 
	if(word_to_translate in dictionary.keys()):
		...
		print(dictionary[word_to_translate]); print("Completed");
		return dictionary[word_to_translate];
else:
print("Not found"); return default_word; #End of the function
response = translate(english_french, "dog", "Not in the dictionary"); print("Response is: ", response);
...
print("Word found, translating...") print(dictionary[word_to_translate]); print("Completed");
return dictionary[word_to_translate];
else:
print("Not found"); return default_word;
#End of the function
response = translate(english_french, "dog", "Not in the dictionary"); print("Response is: ", response);
...
lineCounter = 1; for line in lines: #Using global variable in this scope
global total;
line = line.strip(); #line is the whole line #parts is a list of strings, need to convert each parts = line.split(",");
parts = (list(map(float, parts)));
total += sum(parts);
response = '''\
This is a {lineCounter}
Here is a {total}
'''.format(lineCounter=lineCounter, total="{:1.2f}".format(total)); print(response)
lineCounter += 1; total = 0;
#end of for loop
return total; print(getTotal("scores.csv"));
