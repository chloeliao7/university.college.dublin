...
def find_actors_in_film(doca, out = [])
  ...
  all_tables.each do |tr_node|
    tr_childs_less_newline_nodes = tr_node.children.reject { |node| (node.inner_text == "\n") }
    ...
    if starring_node.inner_text == 'Starring'
      then rest_of_childs = tr_childs_less_newline_nodes[1].children
           rest_of_childs.each { |node| out << node['href'] if node['href'] }
    end
  end
  out.uniq.collect { |link| strip_out_name(link) }
end
...
def strip_out_name(name, nameout = [])
  if name =~ %r{^http://en.wikipedia.org/wiki/}
    nameout = name.gsub(%r{http://en.wikipedia.org/wiki/}, '')
  elsif name =~ %r{^en.wikipedia.org/wiki/}
    nameout = name.gsub(%r{en.wikipedia.org/wiki/}, '')
  elsif name =~ %r{^/wiki/}
    nameout = name.gsub(%r{/wiki/}, '')
