...
class Album
  ...
  def self.build_all(albums = [])
    ...
    # album_names = $songs.values.clean_up
    # p album_names
    # album_names.each { |name| albums << Album.new(name) }
    # make_album(name, tracks, length, artist, owners)
    $songs.each { |song| albums << make_album(song.album, "", song.time, song.artist, song.owners) }
    albums
  end
  ...
  def self.build_an_album_called(_album_name)
    p 'oops...this is missing   ;-)'
