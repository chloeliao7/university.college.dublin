/**
   This class analyzes the distribution of the last digit of values
   from a sequence.
*/
public class LastDigitDistribution
{
   private int[] counters;

   /**
      Constructs a distribution whose counters are set to zero.
   */
   public LastDigitDistribution()
   {
      counters = new int[10];
   }

   /**
      Processes values from this sequence.
      @param seq the sequence from which to obtain the values
      @param valuesToProcess the number of values to process
   */
   public void process(Sequence seq, int valuesToProcess)
   {
      for (int i = 1; i <= valuesToProcess; i++)
      {
         int value = seq.next();
         int lastDigit = value % 10;
         counters[lastDigit]++;
      }
   }

   /**
      Displays the counter values of this distribution.
   */
   public void display()
   {
      /* find largest value in distribution */
      int largest = findMax();

      if (largest == 0) return;

      /* compute scale factor so that the widest bar
         in the histogram is 40 units long */
      double scaleFactor = 40.0 / largest;

      for (int i = 0; i < counters.length; i++)
      {
//         System.out.println(i + ": " + counters[i]);
           System.out.print(i + ": ");
           for (int k = 0; k < counters[i] * scaleFactor; k++)
              System.out.print("*");
           System.out.println();
      }
   }

   /**
      Find the largest distribution value.
      @return largest - the largest number in the counters array
   */
   private int findMax()
   {
      int largest = counters[0];

      for (int i = 1; i < counters.length; i++)
      {
         if (counters[i] > largest)
            largest = counters[i];
      }

      return largest;
   }
}
