...
public class TellerController {
	@FXML
	private TableView<Ledger> tableView;
	@FXML
	private TableColumn<String, String> activityLedger, balanceLedger, rateLedger;
	@FXML
	private Button changeRate, applyInterest, deposit, withdraw, display, exit;
	...
	public void initialize() {
		...
		dialog.setContentText("Enter the starting balance;" + " or press return to use the default constructor");
		...
		do {
			...
			} catch (NumberFormatException nfe) { dialog.setContentText("Enter the starting balance as a number"); }
		} while (!goodBalance);
		...
		if (!defaultObj) {
			...
			do {
				...
				} catch (NumberFormatException nfe) { dialog.setContentText("Enter the interest rate as a positive number"); }
	...
	private void updateLedger(String activity) {
		...
		data.add(new Ledger(activity, money.format(sa.getBalance()), percent.format(sa.getInterestRate())));
	}

	@FXML
	protected void changeRate(ActionEvent event) {
		try {
	...
	@FXML
	protected void applyInterest(ActionEvent event) {
		sa.applyInterest();
		updateLedger("Interest applied");
	}

	@FXML
	protected void deposit(ActionEvent event) {
		try {
	...
	@FXML
	protected void withdraw(ActionEvent event) {
		try {
	...
	private double getInput(String prompt) throws NoInputException {
		...
		do {
			...
			} catch (NumberFormatException nfe) { dialog.setContentText("Value must be a number"); }
		} while (!goodInput);
		return input;
	}
}